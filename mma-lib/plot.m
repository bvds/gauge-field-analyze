...
Options[plotWilsonModelFit] = Join[Options[Graphics3D],
                                   Options[wilsonModel],
                                   {}];
plotWilsonModelFit[data0_, diffFlag_?BooleanQ, opts:OptionsPattern[]]:=
 Block[{bord = 0.025, dims, ff, min, max,
 lco = OptionValue["lowerCutoff"],
 uco = OptionValue["upperCutoff"],
 dx = 1/2,  (* padding in x and y directions *)
 (* Switch around dimensions to minimize number of plots.
    This does not affect the model fitting. *)
 data = Normal[
     KeyMap[If[#[[3]] > #[[4]], #[[{2, 1, 4, 3}]], #]&, data0]],
 popts = Apply[Sequence, FilterRules[{opts}, Options[Plot3D]]],
         ...
         Block[{sdata = 
          ...
          Graphics3D[
              {Map[Block[{fff = #[[2, 1]] - Apply[ff["Function"], #[[1]]]},
                        {{Thickness[0.006], If[fff> 0, Green, Red], 
                          If[Abs[fff] > #[[2, 2]], 
                             Line[{{#[[1, 1]], #[[1, 2]], 0},
                                   {#[[1, 1]], #[[1, 2]], 
                                    If[fff > 0, fff - #[[2, 2]],
              ...
              gopts,
              Axes -> True, 
              AxesLabel -> {Subscript[Style["w", Italic], 1],
                            Subscript[Style["w", Italic], 2],
                            Row[{"\[ScriptCapitalW]","-", Style["W", Italic]}]}, 
              BoxRatios -> {l2/l1, 1, 1}, 
              PlotLabel -> Row[{l1, "\[Times]", l2, " lattice slice"}],
              PlotRange -> {{dx, Min[latticeDimensions] - dx},
                            {dx, Max[latticeDimensions] - dx},
                            All}]]], 
              dims, {1}]],
    ...
    GraphicsColumn[
     ...
     Block[{sdata = 
      ...
      Show[Plot3D[
            ...
            ClippingStyle -> None,
            Mesh -> {Range[Max[latticeDimensions]], 
                     Range[Max[latticeDimensions]]},
            PlotStyle -> If[True, None, Directive[Opacity[0.4]]]],
      Graphics3D[
          Map[Block[{y=ff["Function"][#[[1,1]],#[[1,2]],l1,l2],
                     yy = #[[2,1]], ee= #[[2,2]]},
                    {{Thickness[0.004], Gray, 
                     Line[{{#[[1, 1]], #[[1, 2]], Log[y]},
                           {#[[1, 1]], #[[1,  2]],
                            If[y>yy, Log[yy], Log[yy]]}}]},
             {Thickness[0.008], Blue,
              Line[{{#[[1, 1]], #[[1, 2]], Log[yy + ee]},
                    {#[[1, 1]], #[[1, 2]], Log[yy - ee]}}]}}]&,
              sdata],
        gopts,
        Axes -> True, 
