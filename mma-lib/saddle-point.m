...
applyCutoff3[hess_, grad_, proj_, cutoff_: Pi, zzz_: 1] := 
 Block[{maxLinkShift = Map[shiftNorm, proj], result, count = 0, 
   doPairs = False}, If[doPairs, goodPairs0 = {}; badPairs0 = {}]; 
  result = MapThread[
    If[cutoff zzz Abs[#1] <= Abs[#2] #3, count += 1; 
      If[doPairs, AppendTo[badPairs0, {#1, #2}]]; 0, 
       If[doPairs, AppendTo[goodPairs0, {#1, #2}]]; #2/#1]&,
      {hess, grad, maxLinkShift}]; 
  If[count > 0, 
   Print["applyCutoff3:  ", count, " of ", Length[hess], " zeros"]]; 
  result];
...
cutoffNullspace[hess_, grad_, proj_, cutoff_: Pi, zzz_: 1] := 
  Block[{result, count = 0}, 
   result = 
    MapThread[
     If[zzz < Infinity && cutoff zzz Abs[#1] <= Abs[#2] shiftNorm[#3],
        count += 1; #3, Nothing]&, {hess, grad, proj}]; 
   If[count > 0 || True , 
      Print["cutoffNullSpace:  ", count, " of ", Length[hess], " zeros"]];
   result];
...
Options[dynamicPart] = {Method -> Automatic, printDetails -> True}; 
 ...
 Block[{
  ...
  Which[
   ...
   (* In the dense case, this will be the fastest since the
      ...
      this is quite slow for some unknow reason. *) 
   ...
   OptionValue[Method] === Automatic || 
    methodName[OptionValue[Method]] === "MINRES",
   ...
   Module[{opts = {methodOptions[OptionValue[Method]],
    ...
    printer[Null];
    (# - First[minres[
          Function[x, gaugeTransformShifts.(x.gaugeTransformShifts)], 
          gaugeTransformShifts.#, 
          Apply[Sequence, opts]]].gaugeTransformShifts)&],
   True, 
...
Options[findDelta] = {dynamicPartMethod -> Automatic, 
  ...
  storePairs -> False, largeShiftOptions -> Automatic, 
  storeBB -> False, largeShiftCutoff -> 2,
...
findDelta[hess_, grad_, gaugeTransformShifts_, opts:OptionsPattern[]] := 
 Block[{zzz = OptionValue[rescaleCutoff], 
   ...
   smallProj0 = Block[{vals, vecs,
      (* This estimate is ad-hoc.  
      Perhaps one could use the distribution of pairs? *)
      options = Replace[OptionValue[largeShiftOptions], Automatic -> {}]},
     {vals, vecs} = ersatzLanczos[dp[addTime[tdot, hess.#]]&, Length[grad], 
       Apply[Sequence, options], eigenPairs -> -2 (nc^2 - 1), 
...
latticeSaddlePointStep[opts : OptionsPattern[]] := 
 Block[{hess, grad, delta, tmpGaugeField = makeRootLattice[], 
  ...
  t3 = SessionTime[]; 
  Print["times:  ", {innerLoopTime, t1 - t0, t2 - t1, t3 - t2}]; 
  tmpGaugeField]
