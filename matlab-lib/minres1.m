function [x,flag,iter,relres,relAres,...
	...
	minres1(A,b,rtol,maxit,M,shift,maxxnorm,Acondlim,show)
%MINRES1: min-length solution to symmetric (possibly singular) Ax=b or min||Ax-b||.
%										%   This is the MINRES-QLP code with the QLP part ripped out.
%
%   X = MINRES1(A,B) solves the system of linear equations A*X=B
...
%
%   X = MINRES1(A,B,RTOL,MAXIT,M,SHIFT,MAXXNORM,ACONDLIM,SHOW)
...
%
%
%   [X,FLAG] = MINRES1(A,B,...) returns a convergence FLAG:
...
%      performed.
%
%    [X,FLAG,ITER] = MINRES1(A,B,...) returns ITER,  the
%    number of iterations performed.
%
%    [X,FLAG,ITER,RELRES,RELARES] = MINRES1(A,B,...)
...
%
%    [X,FLAG,ITER,RELRES,RELARES,ANORM,ACOND,XNORM,AXNORM,...
%       RESVEC,ARESVEC] = MINRES1(A,B,...) returns
...
%   x = minres1(A,b,rtol,N,M,shift,maxxnorm,Acondlim,show);
%
...
%         end
%       %-----------------------------------%
% Call MINRES1 using the anonymous function handle defined by Afun.m:
%       b = rand(400,1);
%       x = minres1(@(x,N)Afun(x,20), b);
%
...
if nin <  2 || ~logical(exist('A'       ,'var')) || isempty(A) ...
            || ~logical(exist('b'       ,'var')) || isempty(b)       ,...
                          error('Please provide at least non-empty input for A and b');  
end
...
if nin <  9 || ~logical(exist('show'    ,'var')) || isempty(show)    , show     = true;  end
%if nin< 10 || ~logical(exist('disable' ,'var')) || isempty(disable) , disable  = false; end
...
eta      = 0;      etal     = 0;          etal2    = 0;  % BvdS: initial value of gamal2 overwritten 
...
w        = x;
wl       = x;
% BvdS:  initial value of wl2 overwritten; wl2 may be section-local.
r1       = x;
...
if show
  ...
  fprintf('\nn      =%7g   ||b||    =%10.3e   shift    =%10.3e   rtol     =%10.3e',...
             n,            beta1,             shift,             rtol)
  fprintf('\nmaxit  =%7g   maxxnorm =%10.3e   Acondlim =%10.3e',...
             maxit,        maxxnorm,          Acondlim)
  fprintf('\nprecon =%7g\n' , precon)
  fprintf('\n%s\n', head)
end
...
%% Compute the current left reflection Q_k
  gamal2 = gamal;      gamal    = gama;  % BvdS:  gamal3 not used. 
...
%% Apply the previous right reflection P{k-2,k}
  ...
  if debug   % BvdS:  apply condtional to debug print
...
function p = minresxxxM( M, x )
  ...
  if isa(M,'function_handle')
    p = M(x);
  else  % BvdS:  Split up elseif into two separate lines
