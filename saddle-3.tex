...
the linear confining potential.  In this section, we derive
...
$\hat{\mu}$.  Thus, we sum over the $2^{D-1}$ closest images of the Polyakov
...
\begin{figure}
  ...
  \caption{Polyakov loop correlator vs.\ loop area together with
    ...
    with $L_\mu \left|\mathbf{r}\right|>20$.
...
\be
\mathcal{C}_{i, j} = \cov{\Phi_\mu(\mathbf{r}_i)}{\Phi_\mu(\mathbf{r}_j)} 
\eq
...
\be
\chi^2 = \sum_{i,j} \left(\Phi_\mu(\mathbf{r}_i) - F_\mu(\mathbf{r}_i)\right)
...
$c_1$, $c_p$, and $c_q$ are highly correlated.
...
\be
   ...
   \label{diaghess}
\eq
The other terms of the Hessian follow in a similar fashion.
...
$\mu=1$ even links, then $\mu=1$ odd links,
$\mu=2$ even links, {\em et cetera}.  We will refer to this
...
In this approach, one attempts to find a local minimum
...
\caption{Trajectories for $\SU(4)$, $12 \times 16 \times 20$ lattice,
  ...
  ``15-a'' starts with a different lattice configuration but uses the
   ...
   ``5-sa'' uses the same starting configuration as ``5-a`` but steps
   are calculated with the diagonal color blocks of the Hessian and
     $\gamma=0.05$; 100 steps, every 5th step is plotted.
    \label{trajectory3}}
\end{figure}
...
\caption{Model paramters along the trajectory for the three trajectories
  shown in in Fig.~\ref{trajectory3}.  Note that error bars generally
  decrease as short-length-scale fluctuations are removed from
  the fields. \label{traj4b}
 }
\end{figure}
...
while the short length scale (perturbative) fluctuations are removed.
...
If we define $U^\prime=W^\dag U V$, $U^\prime$ is unitary with some
phase $\theta$: $\det U^\prime = \det V W^\dag = e^{i \theta}$.
We further make the {\em asatz} that $S$ is minimized by choosing $U^\prime$
to be diagonal:
\be
...
In practice, the minimum of (\ref{diagmin}) is computed numerically
using standard library routines.
One can show that if $U_\mathrm{min}^\prime$ is a minimum of
(\ref{diagmin}) then $W U_\mathrm{min}^\prime V^\dag$ is a global minimum
of Eqn.~(\ref{uf}) with respect to $U$.
