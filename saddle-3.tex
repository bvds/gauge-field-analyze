...
the saddle-point.  The goal here is not to make precise
measurements of the string tension and glueball spectra
but to probe that gauge field itself in a more qualitative
fashion for just a single lattice configuration.
...
effective potential at various length scales and will use Wilson loops
and Wilson loop correlators to further probe the system.
...
we fit the correlator $\Phi_\mu(r)$ to an exponential plus a constant term,
...
correction~\cite{teper_large_2009,aharony_effective_2013} in the $\hat{\mu}$
direction as well as the Coulomb terms from Eqn.~(\ref{cpf}):
\be
         c_0 \sum_{\mbox{images}} \exp\left(
         -\sigma a^2 r L_\mu\sqrt{1-\frac{\pi}{3 a^2 \sigma L_\mu^2}}
         - c_q 2 L_\mu \log(r) - c_p 2 L_\mu\right)
...
$\hat{\mu}$.  That is, we sum over the two closest images of the Polyakov
...
various distance scales.  An example for a $16\times 20 \times 24$
lattice is shown in Fig.~\ref{pcorr16}.  The lattice configurations
were generated with the Chroma library~\cite{edwards_chroma_2005};
see Appendix~\ref{configurations} for details.
The string tension $\sigma a^2 = 0.01595(62)$
agrees with Teper's value for $\beta=28$,
$\sigma a^2 = 0.016210(54)$~\cite{lucini_$mathrmsun$_2002,athenodorou_sun_2017}.
The fit has $\chi^2=220$ for 347 degrees of freedom, reflecting
...
We will come back to this minimization later (Table~\ref{center4}).
...
A local minimum of $\left\lVert \config \right\rVert_\zentrum$
can be found via a small modification of the ``Landau step'' discussed
above. That is, when computing the logarithms
in Eqn.~(\ref{landau}) one uses
\be
...
the lattice as described above.
If we apply some number of these ``center steps'' to a lattice
...
That is, for direction $\mu$, we choose a gauge such that $U_\mu(x)$ is replaced
