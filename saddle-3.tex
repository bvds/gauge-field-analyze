...
\begin{table}
  \caption{Parameters for the fit shown in Fig.~\ref{wilsonFit16}.  Note
    that $c_3$ and $c_6$ are not significant.   The string tension is
    highly anti-correlated with the perimeter term $c_p$.
    The string tension $\sigma a^2$ agrees with Teper's value for $\beta=28$,
    $\sigma a^2 = 0.016210(54)$~\cite{lucini_$mathrmsun$_2002,
      athenodorou_sun_2017}.
    \label{wilsonparameters}}
  \begin{tabular}{c|c}
    name & value \\
    \hline
  $\sigma a^2$ & 0.01588(50) \\
  $c_0$ &  1.312(14)\\
  $c_p$ &  0.0671(16)\\
  $c_1$ &  -0.097(15)\\
  $c_2$ &  0.239(15)\\
...
\begin{figure}
  ...
  \caption{Difference of the Wilson loop expectation value $\mathcal{W}$
    ...
    for an $\SU(3)$, $16^3$ lattice, $\beta=28$.  The blue line represents
    the standard error of the data point and the red \& green lines
