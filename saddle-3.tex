...
{\bf  To do: Redo trajectory 2c and 2d (in progress).
  $\SU(3)$, $L^3=20^3$ and
...
\mathrm{separation} =
        \left\lVert \left(x+a \hat{\mu}/2+ a\hat{\nu}/2\right) -
             ...
             \right\rVert \; .
\eq
This correlator corresponds to the $0^{++}$ glueball spectrum.

In Fig.~\ref{corr2dEven}, we plot the correlation for various relative
orientations of the two plaquettes for some point along the
...
the correlator also has contributions from the excited $0^{++}$ states.
...
\be
  ...
  \right\rangle   \; ,
\eq
corresponding to the $0^{--}$ glueballs; see Fig.~\ref{corr2dOdd}.
