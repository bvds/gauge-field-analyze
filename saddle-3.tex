...
\newcommand{\zentrum}{{\mathbb{Z}_N}}       % set of integers
...
\begin{figure}
  \includegraphics{polydist1.pdf}
  \includegraphics{polydistr.pdf}
...
is complex.  In both cases, the map from the $\lambda_i$-space to
...
has multiple local minima.  We are interested here in Landau gauge mainly
...
we find that, to lowest order in the fields,
%
\be
          U_G(x) = e^{- B}
\eq
where
\be
   B = \frac{\zeta}{2 D} \sum_\mu \log\left(U_\mu(x)\right) -
   \log(U_\mu\left(x-a \hat{\mu})\right)  \; . \label{landau}
\eq
One can calculate $U_G(x)$ and apply it at each site of the lattice
in a checkerboard fashion, first for all the even sites and then for
all the odd sites.  This constitutes one minimization step, a ``Landau
step,''
in a sucessive over relaxation (SOR) procedure.
The relaxation factor $\zeta$ can be adjusted to speed numerical
convergence.
In practice we find the fastest convergence for $\zeta \approx 1.7$; however,
it can become unstable for slightly higher $\zeta$.
We generally use $\zeta=1.5$ as a compromise between speed and stability.

\subsection{Maximum center gauge}

\begin{table}
  \caption{Successive applications of ``center steps'' to
...
$\mathbb{I}- \sum_{k} \mathbf{r}_k \otimes \mathbf{r}_k$
