...
\left\lVert \mathbf{w}\right\rVert_2 =
     \frac{1}{L^D D}\sqrt{\sum_{x, \mu, a} w_a^2(\mu, x)}
        \; .
\eq
...
operator $P_G$ onto any Krylov space vector $\mathbf{v}$ using:
\be
...
Use the Lanczos library nu-TRLan~\cite{yamazaki_adaptive_2010},
modified so that $P_G$ is applied to any Krylov space vector, to
find the smallest eigenpairs of $\left(P_g H\right)^2$.  We then find $M$
...
\begin{eqnarray}
   ...
   \left\lVert \mathbf{r}_i\right\rVert_\mathrm{max}}{\left|\lambda_i\right| }
     &>& \Lambda_\mathrm{max} \; .
\end{eqnarray}
To ensure that we have found the eigenpairs that violate the cutoff,
we may need to calculate as many as 2000 to 4000 lowest eigenpairs
in the Lanzos routine.  As one moves along a trajectory towards the saddle point, the gradient vector decreases in magnitude and fewer eigenpairs
