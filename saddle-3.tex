...
\newcommand{\metric}[2]{\left\langle #1,#2\right\rangle}
...
\item The $N\to\infty$ limit of QCD is a good approximation to the
  ...
  the glueball spectrum in both 2+1 and 3+1 dimensions [cite teper].
...
We can define a metric for elements of $\SU(N)$:
%
\begin{eqnarray}
  \metric{U}{V} &=& \sqrt{2 \Tr\,z z^\da} \; , \nonumber\\
  z &=& \log\left(U^{-1} V\right)
    \; , \quad U, V\in\SU(N) \label{metric}
\end{eqnarray}
where the logarithm is defined in Appendix~\ref{roots}.
We can use this metric to define the average
distance between two lattice configurations $\config_k$ and $\config_l$:
\be
   \metric{\config_k}{\config_l} = \frac{1}{L^D D} \sum_{x,\mu}
          \metric{U^{(k)}_\mu(x)}{U^{(l)}_\mu(x)}
\eq
where $U^{(k)}_\mu(x)$ is a link in $\config_k$
and $U^{(l)}_\mu(x)$ is a link in $\config_l$.
...
Since the action is invarient under gauge transforms, $G \mathbf{q} = 0$.
...
imposing the $\Lambda$ cutoff, and using the damping factor $\gamma$,
...
\begin{figure}
\includegraphics{trajectory1}
\includegraphics{trajectory2}
...
\caption{Several trajectories are shown for a $16^3$ lattice configuration,
  ...
  Lattice Monte Carlo; ``distance'' is $\metric{\config_0}{\config_k}$;
  {\bf 2b} was generated with $\gamma=0.1$, $\Lambda_\mathrm{max} = 2$,
  and $\Lambda_2=0.0004$;
  {\bf 2c} was generated with $\gamma=0.2$, $\Lambda_\mathrm{max} = 2$,
  and $\Lambda_2=0.0002$;
...
By applying the procedure dissussed in Section~\ref{saddle} iteratively,
we produce a trajectory of lattice configurations $\config_0, \config_1, \ldots$
in configuration space.  In Fig.~\ref{trajectory123}, we show some trajectories
in terms of the string tension $\sigma a^2$, $\plaquette_\mathrm{avg}$,
and the distance
to the original gauge configuration $\metric{\config_0}{\config_k}$.
...
For the initial configuration $\config_0$ used in Fig.\ref{trajectory123},
...
action is dominated by perbative fluctuations of the gauge fields.
We can use the string tension to
...
Also, the distance to the starting configuration should not become very large.
This is illustrated by the trajectory {\bf 2c}.
...
\eq
This definition of $U^t$ fufills the conditions listed above.
...
color block structure and $(6D-5)$ nonzero blocks per row.
...
operator $P_G$ onto any Krylov space vector $\mathbf{v}$ by solving a
linear system:
\begin{eqnarray}
  P_G \mathbf{v} &=& \mathbf{v} - O^T \mathbf{u} \\
  O O^T \mathbf{u} &=& O \mathbf{v} \label{gcg}
\end{eqnarray}
where Eqn.~(\ref{gcg}) is solved using the conjugate gradient
alogrithm, subroutine MINRES-QLP~\cite{choi_algorithm_2014}.

We start by using Lanczos library nu-TRLan~\cite{yamazaki_adaptive_2010},
modified so that we can apply $P_G$ to any Krylov space vector, to
find the smallest eigenpairs of $H P_g H$.  We then find
eigenvectors $\left\{\mathbf{r}_1, \mathbf{r}_2, \ldots\right\}$
associated with eigenpairs $\left(\lambda_i^2, \mathbf{r}_i\right)$
that violate the $\Lambda$ cutoff,
\be
   \lambda_i^2 \Lambda > \mathbf{r\cdot q}
        \left\lVert \mathbf{q}\right\rVert_2
\eq
(We also use the norm $\left\lVert \mathbf{w}\right\rVert_\mathrm{max}$
to ensure the shift is small for individual links.)
Then we solve the linear system $H \mathbf{y} = \mathbf{q}$
using the conjugate gradient alogrithm,
subroutine MINRES-QLP~\cite{choi_algorithm_2014}, modified so that
$P_G$ and the projection operator
$\mathbb{I}- \sum_k \mathbf{r}_k \otimes \mathbf{r}_r$
